class ConnectFour {
  ...
  def isGridFull: Boolean = fillerIndex.filter(_ < 0).size equals COLUMNS
  ...
  def getAndInsertTile(tile: Tile): Unit = {
    ...
    def isValid: Boolean = position >= 0 && position < COLUMNS && fillerIndex(position) >= 0

    grid(fillerIndex(position))(position) = tile
    fillerIndex(position) -= 1
  }

  def gridToString: String = grid.map("|" + _.map(_.color).mkString("|") + "|").mkString("\n")
